...
Public Class frm_areas
    ...
    Private Sub Btn_cerrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_cerrar.Click
        Me.Close()
    End Sub
    Private Sub Buscar_area()
        Dim comando As New MySqlCommand
        Dim dr As MySqlDataReader 'dr=data reader
        Try
            If chk_mostrar_inactivos.Checked = True Then
                comando.CommandText = "SELECT areas.*, datos_personales.nombres,datos_personales.apellidos FROM (areas inner join datos_personales on datos_personales.num_historia=areas.guardado_por) where area like '%" & txt_busca.Text & "%' order by area"
            Else
                comando.CommandText = "SELECT areas.*, datos_personales.nombres,datos_personales.apellidos FROM (areas inner join datos_personales on datos_personales.num_historia=areas.guardado_por) where area like '%" & txt_busca.Text & "%' and areas.visible=1 order by area"
            End If
            ConectarBD()
            ...
            If dr.HasRows Then
                ...
                Lista_buscar.Columns.Add("Area", 200, HorizontalAlignment.Left)
                ...
                While dr.Read()
                    MyListText(0) = dr("cod_area")
                    MyListText(1) = dr("area")
            ...
            Else
                ...
                Informativo.SetToolTip(btn_buscar, "Buscar el area")
                btn_buscar.Tag = 0
        ...
        Catch Mysql_ex As MySqlException
            Mostrar_mensaje_error_Mysql("Buscar_area", Mysql_ex.Number, Mysql_ex.Message)
        Catch ex As Exception
            Mostrar_mensaje_error("Buscar_area", Err.Number, ex.Message)
        Finally
    ...
    Private Sub Archivar()
        ...
        Try
            If Modificar = False Then
                sql_str = "INSERT INTO areas (cod_area,area,notas,guardado_por,fecha_G, visible) values ('" & _
                txt_codigo.Text & "','" & _
                txt_area.Text & "','" & _
                ...
                MsgBox("Los datos del area " & Chr(34) & txt_area.Text & Chr(34) & " se archivaron correctamente", vbInformation, "Notificación")
                Modificar = True
                Var_cod_area = txt_codigo.Text
                'activo los botones de reporte y eliminar
                btn_eliminar.Enabled = True
                txt_busca.Text = ""
                Buscar_area()
            Else
                sql_str = "update areas set " & _
                "cod_area='" & txt_codigo.Text & "'," & _
                "area='" & txt_area.Text & "'," & _
                ...
                MsgBox("Los datos del area " & Chr(34) & txt_area.Text & Chr(34) & " se modificaron correctamente, se actualizaron (" & Reg_afectados.ToString & ") registro(s) ", vbInformation, "Notificación")
        ...
        Catch Mysql_ex As MySqlException
            Mostrar_mensaje_error_Mysql("Archivar area", Mysql_ex.Number, Mysql_ex.Message)

        Catch ex As Exception
            Mostrar_mensaje_error("Archivar area", Err.Number, ex.Message)
    ...
    Private Sub btn_eliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_eliminar.Click
        Eliminar_generico("areas", "cod_area ", "texto", Var_cod_area, True)
        If Cant_Reg > 0 Then
    ...
    Private Sub frm_tipo_doc_area_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Text = Application.ProductName
        Lbl_titulo.Parent = Pic_Encabezado 'para que quede transparente el label del titulo

        'Buscar()
        Cambiar_Fondo_formulario(Me, Me.Pic_Encabezado, True, Me.Pic_pie_pagina, True, Me.Pic_icono, "Iconos formularios\Icono_frm_areas.png")
    ...
    Private Sub Lista_buscar_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Lista_buscar.SelectedIndexChanged
        If Lista_buscar.SelectedItems.Count > 0 Then
            ...
            txt_area.Text = Lista_buscar.SelectedItems(0).SubItems(1).Text
            ...
            Modificar = True
            Var_cod_area = Lista_buscar.SelectedItems(0).SubItems(0).Text
            lbl_guardado_por.Text = Lista_buscar.SelectedItems(0).SubItems(5).Text
            Mostrar_informacion("inf", "Modo de edición", "Si lo desea cambie los datos, " & vbCrLf & "luego presione archivar para modificarlos", True, 4000, txt_area, "inf")
        End If
    End Sub
